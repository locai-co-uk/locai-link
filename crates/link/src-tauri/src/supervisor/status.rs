...
//! Structured status records emitted on stderr for the host application to
//! parse. Format: one JSON object per line, prefixed with `LOCAI_STATUS:` so
//! the host's parser can ignore non-status log lines on the same stream. The
//! runtime reuses this channel for OTA updates, so one integration covers both
//! bootstrap and update flows.
...
/// Emit a single status line on stderr. Write errors are deliberately
